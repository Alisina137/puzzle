...
export class PuzzleValidator {
  ...
  static validatePuzzle(
    ...
    // 2. Validate all words are placed
  ...
  private static validateGrid(grid: string[][]): PuzzleValidationResult {
    ...
    // Check if grid contains only uppercase letters
  ...
  private static validateWordsPlaced(
    ...
    for (const [word, count] of Object.entries(wordCounts)) {
      if (count > 1) {
        warnings.push(
          'Word "' + word + '" was placed ' + count + " times (duplicate)",
        );
      }
    }
  ...
  private static validatePositions(
    ...
    for (const pw of placedWords) {
      ...
      if (
        ...
        pw.row >= grid.length ||
        pw.col < 0 ||
        pw.col >= grid[0].length
      ) {
        errors.push(
          'Word "' +
            pw.word +
            '" is placed outside grid bounds at (' +
            pw.row +
            ", " +
            pw.col +
            ")",
      ...
      // Check if word matches grid letters
      ...
      for (let i = 0; i < wordLen; i++) {
        ...
        if (r < 0 || r >= grid.length || c < 0 || c >= grid[0].length) {
          errors.push(
            'Word "' + pw.word + '" extends outside grid at position ' + i,
          );
          matches = false;
          break;
        }
        ...
        if (overlapCount > 3) {
          warnings.push(
            'Word "' +
              ...
              '" has ' +
              overlapCount +
              " overlapping letters with other words",
          );
        }
      }
    }
  ...
  private static validateConsistency(
    ...
    // Check if grid has enough filled cells
    let filledCells = 0;
    let totalCells = 0;
    for (let i = 0; i < grid.length; i++) {
      for (let j = 0; j < grid[i].length; j++) {
    ...
    if (fillRatio < 0.3) {
      warnings.push(
        "Grid has low fill ratio: " +
          (fillRatio * 100).toFixed(1) +
          "% (minimum recommended: 30%)",
      );
    }

    // Check if placed words are spread across the grid
    ...
    if (spread < 0.4) {
      warnings.push(
        "Words are clustered together (spread score: " +
  ...
  /**
   * Count overlaps with other words
   */
  private static countOverlaps(
    word: PlacedWord,
    allWords: PlacedWord[],
  ): number {
    let overlapCount = 0;
  ...
  /**
   * Calculate spread of words across the grid
   */
