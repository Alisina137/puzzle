...
export class PDFGenerator {
  ...
  private static recommendTrimSize(maxGridSize: number): string {
    if (maxGridSize <= 12) return "6x9";
    if (maxGridSize <= 16) return "7x10";
    if (maxGridSize <= 19) return "8.25x11";
    return "8.25x11"; // current ceiling for very large grids (20-22+)
  }
  ...
  private static addPuzzlePage(
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fillColor("#1a1a2e")
      .text(`Puzzle #${displayNumber}`, margin, headerY);
  ...
  private static drawSolutionMiniGrid(
    ...
    // ── Adaptive word-list sizing based on actual word count ──
    const rowsNeeded = Math.ceil(words.length / 2);
    let wordFontSize = 6.5;
    let rowH = 9.5;
    if (rowsNeeded > 7) {
      wordFontSize = 5.8;
      rowH = 8;
    ...
    // ── Word list with direction arrows (colors assigned here, reused below) ──
    const listTop = gridStartY + gridPxSize + gapBeforeList;
    const colWidth = boxWidth / 2;
    const half = Math.ceil(words.length / 2);

    // Floor of 5.8pt so the arrowhead geometry in drawDirectionArrow still
    // reads as a triangle rather than a dot at this icon scale.
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      const x = boxX + (isRight ? colWidth : 0);
      ...
      if (info) {
        const angleDeg =
          (Math.atan2(info.direction.dr, info.direction.dc) * 180) / Math.PI;
        this.drawDirectionArrow(doc, x, y + 0.5, arrowSize, angleDeg, color);
      }

      // Numbered to match the puzzle-page word list and the large-grid
      // solution layout, so readers can cross-reference by number
      // everywhere in the book (this list previously had no numbers).
      const label = `${i + 1}. ${word}`;
      const availableWidth = colWidth - arrowSize - arrowGap - 4;
